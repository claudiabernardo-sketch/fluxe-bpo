...
export default function ResetPasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setMsg('Erro ao salvar senha. Tente novamente.')
    }
    setLoading(false)
  }
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'linear-gradient(135deg,#0F0C29,#302B63)', padding:16 }}>
      <div style={{ width:'100%', maxWidth:380, background:'rgba(255,255,255,.07)', borderRadius:20, padding:32, border:'1px solid rgba(255,255,255,.1)' }}>
        ...
        {erro ? (
          <div>
            <p style={{ color:'#fca5a5', fontSize:13, marginBottom:20 }}>Link inválido ou expirado. Solicite um novo link de recuperação.</p>
            <button onClick={()=>nav('/login')} style={{ width:'100%', padding:12, borderRadius:10, border:'none', background:'linear-gradient(135deg,#6366F1,#8B5CF6)', color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer' }}>
              Voltar para o login
            </button>
          </div>
        ) : !ready ? (
