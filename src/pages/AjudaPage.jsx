...
const CATS = [
  {
    ...
    titulo:'Primeiros Passos', artigos:[
      ...
      { titulo:'Criar o primeiro cliente', tempo:'4 min', conteudo:'São 3 passos, nessa ordem, pra esse cliente começar a gerar tarefa sozinho:\n\n1) Acesse o módulo Clientes e clique em "Novo cliente". Preencha nome, CNPJ e contato, e salve.\n\n2) Opcional, mas recomendado: abra o cliente e vá na aba "🔁 Rotina". Cadastre uma rotina pra cada serviço que você presta pra ele (ex: "Conciliação bancária", "Folha de pagamento"), com a frequência (diária, semanal, mensal ou anual) e o horário. Isso ajuda o Fluxe a sugerir o modelo certo sozinho no próximo passo, mas não é obrigatório.\n\n3) Vá em Modelos, selecione esse cliente no filtro do topo. Se você cadastrou rotinas, o Fluxe cruza com os modelos disponíveis (comparando o título) e mostra um botão "Vincular" pra cada par que encontrar. Se não cadastrou rotina nenhuma, ou o título não bateu, use a seção "Outros modelos disponíveis" logo abaixo, que já vem aberta com todos os modelos da biblioteca prontos pra vincular direto. Clique em Vincular em cada um, isso ativa a geração automática das tarefas.\n\nVeja o artigo "Vincular modelos ao cliente" pra entender esse cruzamento em detalhe, e "Biblioteca de modelos prontos" se ainda não importou os 50 modelos já prontos.' },
  ...
  },
  {
    ...
    titulo:'Clientes', artigos:[
      ...
      { titulo:'Aba Onboarding do cliente', tempo:'3 min', conteudo:'Dentro do cadastro do cliente, a aba "Onboarding" guarda os objetivos da parceria, as responsabilidades de cada lado (sua equipe x cliente), o canal oficial de comunicação, o ERP usado e o e-mail padrão — tudo isso já vem com um texto padrão sugerido, só ajustar e salvar.\n\nO "Checklist de onboarding (5 etapas)" (Kick-off & Acessos → Mapeamento → Testes/Validações → Go-live → Acompanhamento/Feedback) é diferente: ao clicar em "+ Criar as 5 tarefas de onboarding", o Fluxe cria 5 tarefas de verdade na categoria Onboarding, pra acompanhar em Tarefas — não é só uma lista decorativa.\n\nSe quiser o passo a passo completo de cada uma dessas etapas (roteiro de kick-off, diagnóstico do cliente, mapeamento de contas etc.) em vez de só as 5 tarefas genéricas, veja o artigo "Biblioteca de modelos prontos" — é só importar e vincular ao cliente.' },
      { titulo:'Diagnóstico financeiro do cliente (link público)', tempo:'3 min', conteudo:'Na aba "Onboarding" do cliente, logo acima do checklist, tem o card "Diagnóstico financeiro do cliente". Clique em "🔗 Copiar link do diagnóstico" e mande esse link pro cliente novo por WhatsApp ou e-mail — ele preenche sozinho regime tributário, porte, faturamento médio, dívidas em aberto, se separa PJ de PF, pró-labore, reserva de emergência, bancos utilizados e se aceita Open Finance, sem precisar de login nem conta no Fluxe.\n\nAssim que ele envia, as respostas aparecem automaticamente nesse mesmo card, com a data de preenchimento — sem você digitar nada de novo. Só os campos financeiros são preenchidos por esse link; objetivos, responsabilidades e canal de comunicação continuam sendo preenchidos pela equipe, no restante da aba Onboarding.' },
      { titulo:'Configurar a Rotina do cliente', tempo:'3 min', conteudo:'A aba "🔁 Rotina", dentro do cadastro do cliente, é onde você registra cada serviço que presta pra ele, é o passo que precisa vir antes de vincular modelos (veja o artigo "Vincular modelos ao cliente"), senão não tem o que cruzar.\n\nClique em "+ Nova rotina" e preencha: Título (dê um nome parecido com o do modelo que você quer usar, por exemplo "Conciliação bancária", "Folha de pagamento", "Fechamento mensal"), Tipo (Diária, Semanal, Mensal ou Anual), e a frequência específica (em Semanal, marque os dias da semana; em Mensal ou Anual, o dia do mês). Um campo de Hora e Observação são opcionais.\n\nCadastre uma rotina pra cada entrega que você faz pra esse cliente. Quanto mais parecido o título da rotina for do título de um modelo existente, mais fácil o Fluxe encontra o par certo no próximo passo.' },
      { titulo:'Vincular modelos ao cliente', tempo:'3 min', conteudo:'Acesse o módulo Modelos e selecione o cliente no filtro do topo da tela. Não é obrigatório ter uma Rotina cadastrada pra vincular um modelo, ela só ajuda o Fluxe a sugerir o par certo automaticamente.\n\nSe o cliente já tem Rotina cadastrada (veja "Configurar a Rotina do cliente"), o Fluxe compara o título de cada Rotina com o título dos Modelos disponíveis (comparação parcial, não precisa ser idêntico, só um conter o outro) e, quando encontra um par, mostra um botão "Vincular". Bolinha verde = já vinculado, laranja = achou um modelo parecido mas ainda não vinculou, cinza = não achou nenhum modelo com título parecido pra essa rotina.\n\nMesmo sem nenhuma Rotina cadastrada (ou se o modelo que você quer não bateu com nenhuma), logo abaixo tem a seção "Outros modelos disponíveis", já aberta, com todos os modelos da biblioteca que ainda não estão vinculados a esse cliente — é só clicar em "Vincular" na linha desejada.\n\nAssim que o primeiro modelo é vinculado, a operação do cliente ativa sozinha, não existe um botão separado de "iniciar operação".\n\nImportante, o processo desse cliente é diferente do processo de outro cliente que usa o mesmo tipo de tarefa (ex: "Contas a Pagar" do cliente A tem passos diferentes do cliente B)? Isso não é motivo pra criar um modelo novo. Vincule o modelo existente e depois use os botões "🔁 Recorrência" e "📋 Checklist" que aparecem ao lado dele (veja "Ajustar recorrência só para um cliente" e "Checklist específico por cliente") pra ajustar dia, horário ou os passos só pra esse cliente, sem tocar no modelo original nem nos outros clientes que usam ele.\n\nSe uma rotina ficar com a bolinha cinza (sem modelo parecido de verdade, não só um processo diferente), você pode criar um modelo novo com o título dela em Modelos, ou renomear a rotina pra bater com um modelo que já existe.' },
      { titulo:'Biblioteca de modelos prontos', tempo:'3 min', conteudo:'Não precisa montar cada modelo do zero. No módulo Modelos, o botão "📚 Importar biblioteca de modelos" traz de uma vez 50 modelos prontos, cobrindo o ciclo inteiro do BPO: Comercial (prospecção, follow-up), Pré-Onboarding (coleta de documentos, regime tributário), Onboarding (reunião de kick-off, mapeamento de contas), Implantação (configuração de ERP, lançamento de saldo inicial), Operacional diário/semanal/mensal (conciliação, pagamentos, cobrança, fechamento, relatório executivo), Estratégico, Acompanhamento (health score, NPS) e Encerramento.\n\nCada modelo já vem com descrição e checklist detalhado — inclusive o roteiro da reunião de kick-off e os pontos de um diagnóstico financeiro completo estão dentro dos checklists dos modelos de Pré-Onboarding e Onboarding, prontos pra virar tarefa. Clique quantas vezes quiser: o Fluxe pula os modelos que você já tiver importado antes (compara pelo título), então não duplica nada. Depois de importar, é só vincular os modelos da etapa certa ao cliente, do jeito normal.' },
      { titulo:'Modelos de contrato e distrato', tempo:'1 min', conteudo:'No menu "Biblioteca", etapa Comercial, tem o "Modelo de Contrato de Prestação de Serviços" e o "Modelo de Distrato de Prestação de Serviços" prontos pra baixar e preencher com os dados do seu BPO e do cliente. O contrato cobre escopo, valor e vigência; o distrato cobre rescisão amigável, quitação de valores pendentes, devolução de acessos e dados, e sigilo.' },
  ...
  },
  {
    ...
    titulo:'Cofre Digital', artigos:[
      { titulo:'Guardar senhas e acessos de clientes', tempo:'2 min', conteudo:'No Cofre, cadastre sistema, login e senha de cada acesso do cliente (banco, ERP, portal da prefeitura, etc.), com criptografia. Assim, quando um analista sai da equipe, ninguém perde acesso a nada.\n\nPra guardar um acesso que não é de nenhum cliente, e sim da sua própria empresa (o sistema contábil do seu BPO, o e-mail da empresa, o banco da empresa), veja o artigo "Cofre da empresa" — é uma aba separada, dentro de Configurações.' },
      { titulo:'Cofre da empresa e quem pode acessar', tempo:'2 min', conteudo:'Em Configurações, a aba "🔐 Cofre da Empresa" guarda os acessos internos do seu próprio BPO (sistema contábil, e-mail, banco da empresa), separados dos acessos dos clientes. Por padrão, só o Admin vê essa aba.\n\nPra liberar outra pessoa da equipe (mesmo que não seja Admin ou Gestor), vá em Configurações > Equipe, clique no lápis (✏) da pessoa, e marque "🔐 Acesso ao Cofre da empresa". Isso libera tanto a aba quanto a permissão de ver as senhas ali dentro, sem mudar o perfil de acesso dela no resto do Fluxe.' },
    ]
  },
  {
  ...
  },
  {
    ...
    titulo:'Mentoria', artigos:[
      ...
      { titulo:'Check-in livre: como você está indo com a mentoria', tempo:'1 min', conteudo:'Dentro da Agenda (e também na aba Mentoria), tem um campo "Como você está indo com a mentoria?", separado dos combinados. É um espaço livre, sem prazo nem sessão específica, pra registrar dúvidas, travas ou evolução quando você quiser. Só sua mentora vê o que você escreve ali.' },
      { titulo:'Calendário e progresso das aulas da turma', tempo:'2 min', conteudo:'Se você faz parte de uma turma da Mentoria em Grupo, o item "📅 Agenda Mentoria" no menu principal mostra um calendário do mês com cada aula marcada no dia certo (fica verde quando você já concluiu). No topo, o card "Próximo evento" já destaca a próxima aula agendada, com um botão verde "🎥 Entrar na aula" que leva direto pro Google Meet daquele encontro, sem precisar copiar link nem entrar em outro lugar. O mesmo calendário também aparece dentro da aba Mentoria, na seção "Aulas da Turma". Clique num dia com aula pra abrir ela, com material, vídeo e exercício. Use os botões "‹" e "›" pra navegar entre os meses, e o botão "📋 Lista" se preferir ver tudo em lista em vez de calendário.\n\nPra marcar uma aula como concluída, clique no círculo numerado dela — ele fica verde com um ✓ — isso alimenta seu progresso, que sua mentora acompanha do lado dela.' },
      { titulo:'Adicionar aulas e combinados ao Google Calendar', tempo:'1 min', conteudo:'Tanto nas aulas da turma quanto nos combinados com sua mentora, existe um botão "📅 Agenda". Clique nele e o Google Calendar abre já com o evento preenchido (título e data certos), você só confirma "Salvar" — não precisa fazer login nem dar nenhuma permissão especial, é só um atalho de preenchimento. Cada mentorada adiciona na sua própria agenda pessoal, independente das outras.' },
    ]
  },
  {
