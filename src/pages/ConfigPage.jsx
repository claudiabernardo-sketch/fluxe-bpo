...
// ── Cofre da Empresa — acessos internos, separados dos acessos de clientes ──
const COFRE_EMP_CATEGORIAS = [
...
function getCofreEmpCat(id) { return COFRE_EMP_CATEGORIAS.find(c=>c.id===id) || COFRE_EMP_CATEGORIAS[4] }

function CofreEmpCopyBtn({ text, label }) {
  ...
  return (
    <button onClick={copy} style={{ padding:'3px 8px', borderRadius:6, border:'1px solid #E2E8F0', background: copied?'#F0FDF4':'#fff', color: copied?'#15803D':'#475569', cursor:'pointer', fontSize:10, fontWeight:600 }}>
...
function CofreEmpresaCard() {
  ...
  const { data: acessos = [], isLoading } = useQuery({
    queryKey: ['acessos_cofre_empresa'],
    queryFn: async () => {
      const { data, error } = await supabase.from('acessos').select('*').is('cliente_id', null).order('sistema')
  ...
  const saveAcesso = useMutation({
    mutationFn: async (dados) => {
      ...
      if (payload.id) {
        ...
        if ('senha_enc' in rest && rest.senha_enc === undefined) delete rest.senha_enc
        const { data, error } = await supabase.from('acessos').update(rest).eq('id', id).select()
        if (error) throw error
        return data
      } else {
        const { data, error } = await supabase.from('acessos').insert({ ...payload, cliente_id: null }).select()
        if (error) throw error
        return data
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['acessos_cofre_empresa'] })
      setModal(null); setForm({}); setRevealed({})
    }
  })
  ...
  const deleteAcesso = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['acessos_cofre_empresa'] })
  })
  ...
  async function save() {
    ...
    if (modal.mode === 'edit') {
      if (form._novaSenha?.trim()) payload._novaSenha = form._novaSenha
      else delete payload.senha_enc
    ...
    <Card>
      ...
      <div style={{ padding:16 }}>
        <div style={{ fontSize:12, color:'#64748B', marginBottom:14, lineHeight:1.6 }}>
          Guarde aqui os acessos da sua própria empresa de BPO (sistema contábil, e-mail, banco), separados dos acessos dos clientes, que ficam no módulo Cofre.
        </div>
        {canSee && (
          <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:12 }}>
            <Btn variant="primary" onClick={openNew}>+ Novo acesso</Btn>
          </div>
        )}
        {isLoading ? (
          <div style={{ textAlign:'center', padding:20, color:'#94A3B8', fontSize:12 }}>Carregando...</div>
        ) : acessos.length === 0 ? (
          <div style={{ textAlign:'center', padding:'30px 20px', background:'#F8FAFC', borderRadius:10, color:'#94A3B8', fontSize:12 }}>
            Nenhum acesso interno cadastrado ainda.
          </div>
        ) : (
          <div style={{ border:'1px solid #F1F5F9', borderRadius:10, overflow:'hidden' }}>
            {acessos.map(ac => {
              const cat = getCofreEmpCat(ac.categoria)
              ...
              return (
                <div key={ac.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', borderBottom:'1px solid #F8FAFC' }}>
                  <div style={{ width:36, height:36, borderRadius:8, background:cat.bg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, flexShrink:0 }}>{cat.icon}</div>
                    ...
                    <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
                      {ac.login && (
                        <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                          ...
                          <CofreEmpCopyBtn text={ac.login} label="login" />
                        </div>
                      )}
                      {ac.url && (
                        <a href={ac.url.startsWith('http')?ac.url:'https://'+ac.url} target="_blank" rel="noreferrer" style={{ fontSize:11, color:'#6366F1', textDecoration:'none' }}>🔗 {ac.url}</a>
                      )}
                  ...
                  <div style={{ textAlign:'right', flexShrink:0 }}>
                    {canSee && ac.senha_enc ? (
                      <div>
                        <div style={{ fontFamily:'monospace', fontSize:12, color:'#0F172A', marginBottom:4, minWidth:100, textAlign:'right' }}>
                          {isLoading_ ? '⏳' : isRev ? revState : '••••••••'}
                        </div>
                        <div style={{ display:'flex', gap:4, justifyContent:'flex-end' }}>
                          <button onClick={()=>toggleReveal(ac)} disabled={isLoading_} style={{ padding:'3px 8px', borderRadius:6, border:'1px solid #E2E8F0', background:'#F8FAFC', color:'#475569', cursor:'pointer', fontSize:10, fontWeight:600 }}>
                            {isLoading_ ? '...' : isRev ? '🙈 Ocultar' : '👁 Revelar'}
                          </button>
                          {isRev && <CofreEmpCopyBtn text={revState} label="senha" />}
                        </div>
                      </div>
                    ) : !canSee && ac.senha_enc ? (
                  ...
                  {canSee && (
                    <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                      ...
                      <button onClick={()=>{ if(confirm('Excluir acesso?')) deleteAcesso.mutate(ac.id) }} style={{ padding:'5px 8px', borderRadius:6, border:'1px solid #FECDD3', background:'#FEF2F2', color:'#991B1B', cursor:'pointer', fontSize:11 }}>🗑</button>
                    </div>
                  )}
                </div>
              )
      ...
      {modal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:16 }}>
          <div style={{ background:'#fff', borderRadius:16, width:'100%', maxWidth:460, maxHeight:'90vh', display:'flex', flexDirection:'column', boxShadow:'0 25px 50px rgba(0,0,0,.15)' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:'1px solid #F1F5F9' }}>
              <span style={{ fontWeight:800, fontSize:15, color:'#0F172A' }}>🔐 {modal.mode==='new'?'Novo acesso interno':'Editar acesso'}</span>
            ...
            <div style={{ padding:'16px 20px', overflowY:'auto', flex:1, display:'flex', flexDirection:'column', gap:12 }}>
              <div>
                ...
                <input value={form.sistema||''} onChange={e=>setForm(f=>({...f,sistema:e.target.value}))} style={fi} placeholder="Ex: Omie, Gmail, Banco Inter..." autoFocus />
              </div>
              <div>
                ...
                <select value={form.categoria||'outro'} onChange={e=>setForm(f=>({...f,categoria:e.target.value}))} style={fi}>
                  {COFRE_EMP_CATEGORIAS.map(c=><option key={c.id} value={c.id}>{c.icon} {c.label}</option>)}
                </select>
              </div>
              <div>
                ...
                <input value={form.login||''} onChange={e=>setForm(f=>({...f,login:e.target.value}))} style={fi} placeholder="usuario@email.com" />
              </div>
              <div>
                ...
                <input type="text" value={form._novaSenha||''} onChange={e=>setForm(f=>({...f,_novaSenha:e.target.value}))} style={fi}
                  placeholder={modal.mode==='edit' && form._temSenha ? '(senha mantida)' : 'Senha ou token de acesso'} />
                {modal.mode==='edit' && form._temSenha && <div style={{ fontSize:9, color:'#94A3B8', marginTop:3 }}>🔒 Senha criptografada armazenada. Digite para substituir.</div>}
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={form.obs||''} onChange={e=>setForm(f=>({...f,obs:e.target.value}))} style={{ ...fi, height:60, resize:'vertical' }} placeholder="Informações adicionais..." />
              </div>
            </div>
